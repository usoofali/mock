...
def extract_questions_and_options(filename):
    ...
    for step in steps:
        ...
        for option_div in option_divs:
            ...
            option_for = option_div.find('input')['id']
            option_text = label.get_text(strip=True) if label else 'No Option Found'
            # option_for = option_for.get_text(strip=True) if label else 'No Option Found'
            options.append(option_text+':id='+option_for)
