...
def get_font_size(draw, text, max_width, max_height, initial_font_size=64, font_path="impact.ttf"):
...
def create_identity_card(photo_path, signature_path, name, adm, department, phone, photo_coords, signature_coords, name_coords, adm_coords, dept_coords, phone_coords, output_path):
    ...
    font_path = os.path.join("font", "impact.ttf") # Adjust the path to the font file if necessary
...
def main():
    ...
    for photo_file in os.listdir(passport_folder):
        ...
        if photo_file.endswith(('.jpeg', '.jpg', '.png')):
            ...
            print(photo_path)
            rem = count % 40
            signature_path = 'Signature/'+str(rem) +'.jpg'
            ...
            if os.path.exists(signature_path):
                ...
                create_identity_card(photo_path, signature_path, name, adm, "COMMUNITY HEALTH", phone, photo_coords, signature_coords, name_coords, adm_coords, dept_coords, phone_coords,output_path)
